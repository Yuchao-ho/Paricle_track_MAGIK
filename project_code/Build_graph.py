...
class Graph_Generator:
    ...
    def compute_connectivity(self, x, frames):
        #positions = x[:, :2]   ## TODO
        positions = x[:, :-1]   ## All Features
        distances = np.linalg.norm(positions[:, None] - positions, axis=-1) / (x.shape[-1] - 2)  # Broadcast
...
class simulate_traj:
    ...
    def __call__(self, particle_feature_pth, prop_steady, num_noise= 1):
        ## simulate traj
        particle_feature_df = pd.read_csv(particle_feature_pth).to_numpy() 
        traj_simu = self.simu_traj(prop_steady)  ##TODO
    ...
    def simu_traj(self,prop_steady):
        ...
        num_steady = int(self.num_particle_sim * prop_steady)
        steady_mat = np.zeros((self.num_frame_sim, num_steady, 2))  # (x,y)
        for idx in range(steady_mat.shape[-1]):
            mean, std_dev = self.len_frame_sim / 2,  self.len_frame_sim / 2
            steady_mat[0, :, idx] = np.random.normal(loc=mean, scale=std_dev, size=num_steady)
            for i in range(1, self.num_frame_sim):
                steady_mat[i, :, idx] = np.random.normal(loc=steady_mat[0, :, idx], scale=1) 
                steady_mat[i, :, idx] = np.clip(steady_mat[i, :, idx], steady_mat[0, :, idx] - 5, steady_mat[0, :, idx] + 5)

        ## stack multistate & steady state
        traj_simu = np.concatenate((traj_simu, steady_mat), axis=1)
        return traj_simu
